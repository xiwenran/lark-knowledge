#!/usr/bin/env python3
"""K6 提示词宪法。

本文件固化 All In Podcast 中文知识库封面与内页海报的 V2 prompt 模板，
施工依据为 docs/skill-allin-upgrade.md 第 5 章（K6: 提示词宪法）。
"""

from __future__ import annotations


COVER_V2_TEMPLATE = """你是一位顶级海报艺术家，正在创作 All In Podcast 中文知识库的封面海报。
风格基底：手绘水彩 + 剪纸拼贴 + 东方艺术质感（参考国风/日系艺术海报传统），
而不是数字插画、商业插画或字效海报。整体气质要像一张**展览级艺术作品**，
有印刷品质的颗粒感、纸张纹理、笔触温度，让人愿意挂在墙上。

━━━━━ 一、深度词义转译 ━━━━━
核心词「{core_word}」在本期 All In Podcast {episode} 的语境中，
{context}
本期关键内容：
{points}
情绪气质：宏大转折、命运感、新王登基、旧秩序崩解、咽喉关口的争夺。

{core_word_symbolism}

━━━━━ 二、画面整体场域 ━━━━━
统一视觉场，不分栏拼贴。文字、图像、辅助信息、留白共生。
有自然阅读流向：先被巨型主标题击中 -> 被主体隐喻吸引 -> 自然看到角落补白信息。

━━━━━ 三、主标题系统 ━━━━━
巨大的中文主标题「{core_word}」是绝对主体，占画面 40-60% 面积。
**字形必须清晰、完整、可识别，不要出现错字、缺笔、变形**--
建议采用书法字、剪纸字、水墨字或宋体艺术化处理，但保证笔画完整。
两个字可以采用不同质感（如「算力」用厚重墨色、「入口」用透雕剪纸效果），
形成结构对比与视觉张力。

━━━━━ 四、视觉隐喻（择其一最强者）━━━━━
建议核心隐喻方向：
{metaphor_options}

挑选最有命运感和戏剧张力的方向，保证一眼能读懂寓意。

━━━━━ 五、必要封面信息（嵌入式排版，不分栏）━━━━━
以下信息要自然嵌入画面，不形成独立信息栏：
- 顶部或侧栏小字：「ALL IN PODCAST 中文知识库」
- 期号 + 日期：「{episode} · {date}」
- 播放量 + 时长 + 主题：「{duration} · 播放量 {views} · {topic}」
- 副标题（可选嵌入下方留白处）：「{title}」
- 关键词条（如剪纸标签或印章效果）：「{sub_words}」
- 四位主播名：Jason · Chamath · Sacks · Friedberg（小字嵌入侧边或底部，像署名一样克制）

排布原则：所有这些信息要像贴在墙上的展签、印章、批注一样自然，
不能整齐对齐成 form 表单状，要错落有致、有节奏。

━━━━━ 六、辅助诗性补白（少量、嵌入式）━━━━━
在画面留白处放 1-2 句诗性短语（不要多）：
{aux_poetry}
位置可在角落、贴着主体、或主标题边缘。要像低语、批注、墨迹，
不要做成说明栏。可以用毛笔字或印章式排布。

━━━━━ 七、色彩逻辑（克制 3-4 色）━━━━━
推荐配色（本期选定色组 {palette}，按 poster_template.py 的 COLOR_PALETTES 配色矩阵执行）：
整体气质要"高级、克制、展览级、印刷品质感"。
不要俗艳拼色、不要数字渐变、不要商业插画风。

━━━━━ 八、风格质感 ━━━━━
强烈倾向：手绘水彩 + 剪纸拼贴 + 石版印刷 + 丝网印刷 + 纸张颗粒 + 笔触温度。
画面应有"被人手做出来"的痕迹，而非"AI 生成"的光滑感。
允许：水墨晕染、纸纤维肌理、印章压痕、剪纸边缘、轻微噪点、做旧感。

━━━━━ 九、严格禁止 ━━━━━
- 不要把信息分成多个独立的卡片/栏目
- 不要堆群像（最多 1-2 个微小人影/主体）
- 不要做单纯字体海报（必须有主体演绎和视觉转译）
- 不要出现错字、缺笔、变形字、英文界面元素、页码、版本号、版权声明
- 不要俗艳渐变、廉价模板感
{forbidden}

━━━━━ 输出 ━━━━━
竖版 3:4 比例。整张海报必须看起来像「策展级艺术作品」，
有意境、有情绪、有命运感，并且让观者一眼明白本期讨论的是什么。
"""


INNER_V2_TEMPLATE = """你是一位顶级海报艺术家，正在创作 All In Podcast 中文知识库的**内页**。
风格基底：手绘水彩 + 剪纸拼贴 + 东方艺术质感（参考国风/日系艺术海报传统），
保留"词义转译+图文融合+意境"的概念海报基因，
同时拥抱信息图的密度--像一张精心装裱的"主题图谱"，
每翻到一页都让读者感到内容饱满、视觉有惊喜，但不挤压、不混乱。

━━━━━ 一、章节定位 ━━━━━
本页主题：「{page_title}」
核心隐喻词：「{core_keyword}」（贯穿全页的视觉母题）
副标题：「{page_subtitle}」

整张内页要围绕「{core_keyword}」这个核心隐喻展开--
{core_keyword} 的剪影、轮廓、阴影或痕迹以装饰性方式贯穿画面，作为整页的视觉母题。
{cross_page_motif_hint}

━━━━━ 二、版面结构（你自由选择，不强加固定布局）━━━━━
版面由你根据本页主题自由选择最合适的方案，可参考但不限于：
- 纵向时间线（适合议题演进、因果链）
- 横向并列分栏（适合对照、比较）
- 三角放射 / 中心+卫星（适合一个母题+多个支柱）
- 错落散点云（适合多个独立观点共存）
- L 形 / U 形 / 网格分组（适合分类展示）

评价标准：**美观、协调、信息密度饱满但不挤压、阅读节奏自然**。
不要硬套上述某一种，按本页核心隐喻和 3 个要点的关系挑最贴切的版面。

━━━━━ 三、信息密度要求（核心升级）━━━━━

**整页约 12-18 个独立视觉单元**（含标签、图标、关键词条目、说明文字、装饰），
比单纯的"标签+一句话"密度大幅提升，但不超过 18 个以免手机查看拥挤。

**每个要点的视觉构成（必须完整呈现以下五要素）**：

1. **醒目标签**：要点 label，用毛笔字或剪纸字效突出
2. **核心图标** 1 个：从 icon_hint 提示中选最贴切的 1 个手绘图标
3. **3-5 个关键词条目**：**必须**从该要点的说明文字（text）中主动识别名词性短语和专有名词作为视觉条目。
   - 例：text="SpaceX 收购 Cursor 不是为了订阅收入，而是以编程工具为入口向企业输送算力，复制 AWS 生态路径"
   - 提炼条目：`SpaceX` / `Cursor` / `订阅制` / `企业算力` / `AWS 路径`（5 个名词性短语）
   - 表现形式任选：圆点列表 / 印章式标签 / 小卡片 / 连接节点 / 散点标注，每个要点统一一种
4. **1 句精炼说明**：保留原 text 的核心断言，截短到约 30-40 字，作为该分区的注解
5. **1-2 个装饰性元素**（不堆砌）：手绘小图标 / 印章 / 墨点 / 连接线 / 星标 / 注解箭头，配合核心隐喻母题

要点之间用墨迹连线、箭头、视觉过渡或留白节奏自然衔接，不要做成 3 个独立卡片。

━━━━━ 四、手机屏幕阅读约束（重要）━━━━━

画面输出 1024×1536 竖版（3:4），用户主要在 **iPhone 屏幕上查看**（物理宽度约 71mm，缩放后实际显示宽度约屏幕 90%）。

因此必须满足：
- **字号下限**：主标签字号 ≥ 整图宽度的 4%（约 40px@1024 宽）；关键词条目字号 ≥ 2.8%（约 28px）；说明文字字号 ≥ 2.2%（约 22px）
- **不要做极细长的水平条**（手机上看不见）
- **不要在画面边缘 5% 范围内放重要文字**（手机查看时可能被裁切或挤压）
- **整页独立视觉单元 ≤ 18 个**，否则手机查看会拥挤

━━━━━ 五、要点内容（参数注入）━━━━━
{points}

按上面"信息密度要求"的五要素逐个渲染。
注意：每个 point 的 text 是给你的素材库，**你要从中提炼关键词条目**，不要把整段 text 当成唯一渲染内容。

━━━━━ 六、视觉一致性 ━━━━━
- 与封面海报使用相同的色彩语言（本期色组 {palette}）
- 手绘水彩 + 剪纸拼贴质感
- 字体和封面保持一致（毛笔/书法/宋体艺术化）
- 同一期 4 张内页保持类似的版面节奏（不要每张换截然不同的版式风格）

━━━━━ 七、辅助文字 ━━━━━
允许在画面留白处加 1 句诗性短语（贯穿全页主题）：
{aux_poetry}
位置克制，像题跋或落款一样。

底部右下角小字：「All In 中文笔记」（系列标识）

━━━━━ 八、严禁 ━━━━━
- 不要做成 3 个矩形信息卡的拼接（必须有视觉融合）
- 不要把 text 整段渲染（必须提炼出 3-5 个关键词条目）
- 不要超过 18 个独立视觉单元（手机查看会拥挤）
- 不要出现错字、缺笔、变形字
- 不要英文界面元素、页码、版本号
- 不要俗艳渐变、廉价模板感
- 不要让画面看起来像封面（必须有内容密度，不只有大字）

━━━━━ 输出 ━━━━━
竖版 3:4 比例。让读者在手机上翻到这页时感到：
画面有意境、信息有重量、关键词清晰可读、阅读有节奏、视觉有惊喜。
"""


COLOR_PALETTES = {
    "A": {
        "name": "轻盈派（亮色默认）",
        "description": "粉蓝 #4A90E2 + 暖黄 #F5C842 + 米白 #F8F4ED + 深墨 #2D2D2D",
        "primary": "#4A90E2",
        "accent": "#F5C842",
        "bg": "#F8F4ED",
        "ink": "#2D2D2D",
        "secondary": "#A8C4E0",
        "scene_keywords": ["默认", "轻盈", "产品", "增长", "消费", "工具", "乐观"],
    },
    "B": {
        "name": "清新派（亮色备选）",
        "description": "薄荷青 #5DA68F + 杏粉 #E8B197 + 米白 #F8F4ED + 深墨 #2D2D2D",
        "primary": "#5DA68F",
        "accent": "#E8B197",
        "bg": "#F8F4ED",
        "ink": "#2D2D2D",
        "secondary": "#B8D8CF",
        "scene_keywords": ["清新", "医疗", "教育", "协作", "社区", "生活方式"],
    },
    "C": {
        "name": "东方庄重派（深色，重型题材）",
        "description": "朱红 #C73E2C + 暖金 #C8A35F（小面积）+ 米黄宣纸 #F5F2EA + 墨黑 #1A1A1A",
        "primary": "#C73E2C",
        "accent": "#C8A35F",
        "bg": "#F5F2EA",
        "ink": "#1A1A1A",
        "secondary": "#8B2018",
        "scene_keywords": ["债务", "危机", "战争", "博弈", "破产", "颠覆", "威胁", "垄断"],
    },
    "D": {
        "name": "藏青现代派（深色，命运感题材）",
        "description": "藏青 #1B365D + 暖金 #C8A35F + 米白 #F8F4ED + 朱砂红印章 #C73E2C（小面积）",
        "primary": "#1B365D",
        "accent": "#C8A35F",
        "bg": "#F8F4ED",
        "ink": "#1A1A1A",
        "secondary": "#C73E2C",
        "scene_keywords": ["五星", "命运感", "算力", "能源", "芯片", "AI", "基础设施", "监管"],
    },
}


METAPHOR_LIBRARY = {
    "算力入口": [
        "A. 微小人影站在巨型「入口」字下方光中，门内是无限纵深空间",
        "B. 「算力」字底部是数据中心建筑群剪影，「入口」字内透出朱红色光",
        "C. 手绘特洛伊木马剪影从「算力」字内驶出，朝向远方城市",
        "D. 巨型字下方是水墨晕染的通道场景，一两个手绘小角色驻足前方",
    ],
    "关税": [
        "A. 港口闸门前堆叠的集装箱被朱红关卡切开",
        "B. 一张巨型税票像城墙一样横在贸易航线上",
        "C. 货船穿过墨色海峡，船身被金色刻度重新标价",
        "D. 宣纸上的贸易路线被一道红印截断，留出大片空白",
    ],
    "AI监管": [
        "A. 透明牢笼中悬浮的神经网络被朱红印章封存",
        "B. 巨型法槌投下阴影，阴影内部是芯片和数据流",
        "C. 一只水墨眼睛凝视算法迷宫，入口处挂着监管封条",
        "D. 蓝黑色云层下，细密规则线条缠绕发光模型核心",
    ],
    "开源": [
        "A. 打开的木刻机关盒向外散出代码纸片和工具零件",
        "B. 多条溪流汇入同一张宣纸河网，形成共享源头",
        "C. 被打开的城门内有许多手绘小人共同搬运模块",
        "D. 墨色树根向四周蔓延，每条根须都挂着小标签",
    ],
    "数据主权": [
        "A. 数据河流被边界碑分割，水面上漂浮朱红印章",
        "B. 巨型钥匙插入服务器城堡，城墙刻着主权纹样",
        "C. 地图轮廓下方埋着发光数据矿脉，边境线清晰可见",
        "D. 墨色云库被金色锁链圈住，只露出少量光点",
    ],
    "能源": [
        "A. 发电站烟囱化作毛笔笔触，连接远处数据中心",
        "B. 金色电流穿过山脉和海岸，最终汇入芯片纹路",
        "C. 太阳、风机、核电站以剪纸层次围绕一颗发光核心",
        "D. 暗色宣纸上只有一条暖金电弧照亮基础设施轮廓",
    ],
    "芯片": [
        "A. 巨型芯片像古代城池俯瞰平原，电路线成为护城河",
        "B. 硅晶圆被画成月亮，月面刻着细密电路",
        "C. 手绘工匠在金色芯片边缘雕刻，象征精密制造",
        "D. 墨色芯片剪影中透出朱红光点，像未开启的引擎",
    ],
    "加密货币": [
        "A. 金色硬币漂浮在墨色漩涡上，外圈是区块链刻痕",
        "B. 古代钱庄与数字钱包的剪影重叠，形成新旧金融对照",
        "C. 一条由方块组成的桥跨过深谷，桥下是法币纸片",
        "D. 深蓝夜空中散落发光账本节点，彼此以细线相连",
    ],
    "商品引流": [
        "A. 实物链接像细线一样从商品样本延伸到多个内容入口",
        "B. 钩子封面像剪纸门牌悬在商品前方，吸引视线进入",
        "C. 钩子标题化作手写路标，引导人群走向同一个转化口",
        "D. 漏斗下方承接商品盒子，流量颗粒被筛成购买线索",
    ],
    "私域转化": [
        "A. 阶梯从公域入口逐级通向私域小屋，层层标出信任变化",
        "B. 池塘中央漂着商品样本，四周水纹代表持续触达",
        "C. 链条把内容、私信、社群和成交节点串成闭合路径",
        "D. 闭环像手绘圆环包住用户旅程，末端回到复购入口",
    ],
    "选品逻辑": [
        "A. 天平一端放需求信号，一端放供应成本，商品站在平衡点",
        "B. 滤网筛过一组候选商品，只留下少数高匹配样本",
        "C. 罗盘指向细分赛道，刻度上标着价格、需求和交付难度",
        "D. 候选墙上贴满商品剪影，少数被朱红印章圈出",
    ],
    "内容入口": [
        "A. 标题像大门横梁，用户从关键词铺成的小路走入画面",
        "B. 封面成为明亮橱窗，商品与卖点在窗内被重新排列",
        "C. 关键词矩阵像棋盘展开，每个格子都是一条可测试入口",
        "D. 钩子像鱼钩形剪纸符号，从信息流中拉出注意力",
    ],
    "商业模式": [
        "A. 流转图把流量、产品、交付和复购画成一条水墨循环线",
        "B. 引擎剖面中嵌着价格标签、内容模块和成交齿轮",
        "C. 链路从曝光节点延伸到交付盒子，每段都有手绘刻度",
        "D. 转盘分成获客、转化、交付、复购四格，商品位于中心",
    ],
    "流量拆解": [
        "A. 漏斗分层承接曝光、点击、私信和下单，颗粒逐层变少",
        "B. 路径像地图航线一样穿过多个内容节点，最终抵达商品",
        "C. 入口以多扇小门呈现，每扇门对应一种内容钩子",
        "D. 触点像印章散布在用户旅程上，连成可复盘的转化线",
    ],
}


def pick_palette(record: dict | str) -> dict:
    """根据飞书记录内容自动选择配色，支持 forced_palette 强制覆盖。"""
    if isinstance(record, str):
        forced_palette = record.strip().upper()
        if forced_palette in COLOR_PALETTES:
            return COLOR_PALETTES[forced_palette]
        record = {}

    forced_palette = str(record.get("forced_palette", "")).strip().upper()
    if forced_palette in COLOR_PALETTES:
        return COLOR_PALETTES[forced_palette]

    title = str(record.get("cn_title") or record.get("title") or "")
    topic = str(record.get("topic") or record.get("主题分类") or "")
    text = f"{title} {topic}"
    score = record.get("five_dim_score", 0)

    heavy_keywords = [
        "债务",
        "危机",
        "战争",
        "博弈",
        "算力争夺",
        "洗牌",
        "破产",
        "颠覆",
        "威胁",
        "争夺",
        "入侵",
        "垄断",
    ]
    destiny_keywords = ["算力", "能源", "芯片", "AI监管", "监管", "数据主权", "基础设施"]
    fresh_keywords = ["医疗", "教育", "协作", "社区", "生活方式"]

    if any(keyword in text for keyword in heavy_keywords):
        return COLOR_PALETTES["C"]
    if score == 5 or any(keyword in text for keyword in destiny_keywords):
        return COLOR_PALETTES["D"]
    if any(keyword in text for keyword in fresh_keywords):
        return COLOR_PALETTES["B"]
    return COLOR_PALETTES["A"]


def render_cover_prompt(params: dict) -> str:
    """Render the cover poster prompt from explicit template parameters."""
    return COVER_V2_TEMPLATE.format(**params)


def render_inner_prompt(params: dict) -> str:
    """Render the inner-page poster prompt from explicit template parameters."""
    return INNER_V2_TEMPLATE.format(**params)


if __name__ == "__main__":
    cover_params = {
        "episode": "E270",
        "date": "2026-04-24",
        "core_word": "算力入口",
        "points": "- 工具是入口\n- 算力是终局\n- 第四朵云崛起",
        "palette": "D",
        "aux_poetry": "- 「工具是入口，算力是终局」\n- 「咽喉之争」",
        "forbidden": "",
        "context": "讨论 SpaceX 申请收购 AI 编程工具 Cursor，把编程工具当作切入企业算力市场的入口。",
        "core_word_symbolism": "「算力」象征庞大、坚硬、稀缺的基础设施；「入口」象征狭窄、关键、决定性的关卡。",
        "metaphor_options": "\n".join(METAPHOR_LIBRARY["算力入口"]),
        "sub_words": "特洛伊木马·债务炸弹·第四朵云",
        "title": "SpaceX收购Cursor：算力入口重构产业链",
        "duration": "90分钟",
        "views": "44万",
        "topic": "科技&AI",
    }
    assert render_cover_prompt(cover_params) == render_cover_prompt(cover_params)
    print("PASS")
