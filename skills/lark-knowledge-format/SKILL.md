...
⚠️ **禁止嵌套**：`<text>` 标签内部不能再出现 `<text>`，无论属性是否相同。

**场景 A — 同一段文字既要字色又要背景色**：用单标签两属性。
- ❌ `<text color="red"><text background-color="light-red">文字</text></text>`
- ✅ `<text color="red" background-color="light-red">文字</text>`

**场景 B — 整句背景色 + 内部不同词不同字色**（高频场景，容易写成嵌套）：把整句拆成多个**相邻的非嵌套片段**，每个片段独立闭合。纯文本片段只带背景色，着色词片段用单标签组合字色+背景色。
- ❌ 嵌套（错误）：`<text background-color="light-purple">"请<text color="red">举手</text>。"</text>`
- ✅ 分段式（正确）：`<text background-color="light-purple">"请</text><text color="red" background-color="light-purple">举手</text><text background-color="light-purple">。"</text>`

分段式写法原则：**每个 `<text>` 标签必须是叶子节点**——开标签和闭标签之间只有纯文本，没有任何其他标签。
...
2. ✅ **无嵌套 `<text>` 标签**：任何 `<text>` 开闭标签之间只能有纯文本，不能有另一个 `<text>`
   - **场景 A（同词双属性）**：❌ `<text color="red"><text background-color="light-red">文字</text></text>` → ✅ `<text color="red" background-color="light-red">文字</text>`
   - **场景 B（整句背景色+内部多色词）**：❌ `<text background-color="light-purple">"请<text color="red">举手</text>。"</text>` → ✅ 拆成分段式：`<text background-color="light-purple">"请</text><text color="red" background-color="light-purple">举手</text><text background-color="light-purple">。"</text>`
   - **自检方法**：全文搜索 `<text`，检查每个开标签到对应闭标签之间有没有出现另一个 `<text`，有就是嵌套
...
**核心原则**：与通用知识库排版规则**完全独立**。排版目标是「结构清晰，说话人一眼认出，高价值内容突出」。**不改一个字**，只做格式重排与颜色标注。
...
```
<text color="grey">HH:MM</text> **<text color="[说话人色]">说话人</text>**
...
| 说话人 | 角色 | 颜色标签 |
|--------|------|---------|
| Jason | 主持人 | `<text color="blue">` |
| Chamath | 联合主持 | `<text color="green">` |
| Sacks | 联合主持 | `<text color="orange">` |
| Friedberg | 联合主持 | `<text color="purple">` |

**规则**：全篇同一说话人始终用同一颜色，绝不混用。

---

#### 三、逐字稿关键词着色（颜色密度：每段 1-3 个，不多）

| 类型 | 颜色 | 示例 |
|------|------|------|
| 公司/产品/人名 | `blue` | Cursor、SpaceX、Thoma Bravo |
| 数字/金额/比例 | `red` | $64亿、NRR 80%、年增 20% |
| 判断/结论 | `orange` | "这是零和游戏"、"已不可逆" |
| 正向机会/增长 | `green` | 三倍增长、先发优势 |
| 术语/框架 | `purple` | LBO、Agent 治理、表观基因组 |

**禁止**：口水词、过渡词、普通动词不着色；不做装饰性堆色。

---

#### 四、背景色规则（仅用于高价值句子，不滥用）

| 背景色 | 用途 | callout 标签 |
|--------|------|-------------|
| light-yellow | 金句/核心判断 | `<callout background-color="light-yellow">` |
| light-red | 强结论/风险警告 | `<callout background-color="light-red">` |
| light-blue | 方法论/框架解释/注释 | `<callout background-color="light-blue">` |
| light-green | 行动建议/可执行项 | `<callout background-color="light-green">` |
| grey | 备注/待核实/含糊表述 | `<callout background-color="grey">` |

**Callout 只包**：真正值得二次加工的句子（金句、结论、行动建议），不是每段都加，逐字稿全文 callout 总数控制在 ≤ 15 条。
...
| 区域 | 处理方式 |
|------|---------|
| 页面头部（期号行 + 📌 摘要） | 公司名 `blue`，数字 `red` |
| light-yellow 概览 callout | **原封不动** |
| 五维分析各节 | 关键数字 `red`，公司名/人名 `blue`，核心结论句 callout light-yellow |
| 精华金句 | 说话人名用对应专属颜色，引号内关键词 `red` |
| 章节标题（## / ###） | `<text color="blue">` 包裹 |
| 注释 callout | **原封不动**，已格式化 |
...
逐字稿内每个时间段的标题保持：
```
**[HH:MM–HH:MM] 章节主题（中文，5字以内）**
```
不加额外颜色，保持粗体即可（避免与说话人颜色混淆）。
...
- [ ] 每段着色词是否 ≤ 3 个？
- [ ] callout 总数是否 ≤ 15 条？
- [ ] 没有把 > **Speaker**: 原文 的 blockquote 格式留在文档里（会产生灰色竖线）？
- [ ] 英文和中文是否各自独立成段（不在同一行）？
- [ ] 五维分析里关键数字是否都有 red 标注？
