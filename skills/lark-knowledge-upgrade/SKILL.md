...
**视觉风格基底**（所有图共用，复用 All In 手绘笔记风格）：

> 手绘 sketchnote 笔记风格。黑色线条为主，天蓝色（#4DABF7）作为高亮强调色，米白色背景（#FAFAF7）。手写字体质感，有涂鸦式图标和指示箭头。竖版 3:4 比例，内容居中，留白充足，最多 4 个主要视觉元素。底部右下角标注小字：「产品拆解笔记」。不要出现任何英文界面元素，全部用中文。不要添加页码标识。
...
python3 ~/lark-knowledge/scripts/shared/gen_image.py \
  --prompt "<风格基底 + 该页内容提示词>" \
  --output "/tmp/sketchnote_<记录编号>_0N_<主题>.png" \
  --size "1024x1536"
```
